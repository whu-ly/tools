...
static void adjweek(rtcm_t *rtcm, double tow)
{
    ...
    if (week != myweek) week = myweek; // edit by Lying
...
static int obsindex(obs_t *obs, gtime_t time, int sat)
{
    ...
    if (i>=MAX_OBS) return -1; /* overflow */
    
    /* add new field */
    obs->data[i].time=time;
    obs->data[i].sat=sat;
    for (j=0;j<N_FREQ+NEXOBS;j++) {
...
static int decode_type1002(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAX_OBS&&i+74<=rtcm->len*8;j++) {
...
static int decode_type1004(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAX_OBS&&i+125<=rtcm->len*8;j++) {
        prn  =getbitu(rtcm->buff,i, 6); i+= 6;
...
static int decode_type1010(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAX_OBS&&i+79<=rtcm->len*8;j++) {
        prn  =getbitu(rtcm->buff,i, 6); i+= 6;
        code =getbitu(rtcm->buff,i, 1); i+= 1;
...
static int decode_type1012(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAX_OBS&&i+130<=rtcm->len*8;j++) {
        prn  =getbitu(rtcm->buff,i, 6); i+= 6;
        code1=getbitu(rtcm->buff,i, 1); i+= 1;
        fcn  =getbitu(rtcm->buff,i, 5); i+= 5; /* fcn+7 */
        pr1  =getbitu(rtcm->buff,i,25); i+=25;
...
static int decode_type1019(rtcm_t *rtcm)
{
    ...
    if (myweek != eph.week)myweek = eph.week; // edit by Lying
    //if (rtcm->time.time==0) rtcm->time=utc2gpst(timeget());
...
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    if ((myweek - 1356) != eph.week)myweek = eph.week + 1356; // edit by Lying
    //if (rtcm->time.time==0) rtcm->time=utc2gpst(timeget());
                     ...
                     int *idx)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (idx[i]>=N_FREQ) { /* save as extended signal if idx >= N_FREQ */
            ex[i]=1;
            continue;
        }
        /* code priority */
    ...
    for (i=nex=0;i<n;i++) {
        if (ex[i]==0) ;
        else if (nex<NEXOBS) idx[i]=N_FREQ+nex++;
